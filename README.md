...
- The 15fps cap (`videorate max-rate=15`) sits **before** the 1080p→640×360 `videoscale`, which is the whole cost of this branch. Scaling first and dropping frames afterwards — the original order — made the scale run at the full 60fps. **Measured on the box against a live 1080p60 feed, this one reordering took the whole always-on pipeline from 128% of one core to 74%** — over half a core, for identical output (preview delivery 14.5 vs 14.4fps). Isolated to the branch itself: 48–50% of one core versus 17.8%. On the slower A55 cores the original order couldn't sustain 60fps at all.
- `max-rate` rather than a fixed `framerate=15/1`, because `videorate` in that position also duplicates frames *upward* to hit a fixed rate. Standby feeds the tee at 10fps, so a pinned 15 would invent 5 frames a second and scale them at 1080p: 8.3% of a core against 2.3% for `max-rate`, paid continuously on a fanless box that idles between flights. Standby therefore previews at a true 10fps, live at 15 — verified in the browser at 10.0fps and 14.5fps respectively.
